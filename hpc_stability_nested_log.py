...
# forward alg, matrix multiplication in log space
...
os.chdir(sub_folder)
from _nested_for_loops_log import nested_for_loops_log
...
def compute_loss_and_grads(A, B, pi, u, U):
    ...
    with tf.GradientTape() as tape:
        ...
        alpha = nested_for_loops_log(A, B, pi, u, U)  # (batch x K x L)
...
comp_time_0_save = np.zeros((len(one_hot_batch_list), num_of_repeat_slow_alg), dtype = float)  
gpu_list_0_save = []
ram_list_0_save = [] 
...
gpu_monitor_thread = threading.Thread(target = gpu_ram_measure, 
                                      args = (gpu_list_0_save, ram_list_0_save, num_measure_slow_alg, stop_event))
...
try:
    ...
    for i in range(len(one_hot_batch_list)):  
        ...
        for r in range(num_of_repeat_slow_alg):  
            ...
            comp_time_0_save[i, r] = end - start
            
            # Print progress message
            tf.print(f'Nested Log Version, Input Length: {L}, Repetition {r + 1} of {num_of_repeat_slow_alg} completed.')
        #tf.print(alpha)
        #tf.print(grads)
...
save_data(save_dir, comp_time_0_save, gpu_list_0_save, ram_list_0_save, 'nested_log')