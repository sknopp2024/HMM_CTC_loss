...
os.chdir(sub_folder)
from _matrix_mult_log_tf import matrix_mult_log
...
def compute_loss_and_grads(A, B, pi, U):
    ...
    with tf.GradientTape() as tape:
        ...
        alpha = matrix_mult_log(A, B, pi, U)  # (batch x K x L)
...
gpu_monitor_thread = threading.Thread(target = gpu_ram_measure, 
                                      args = (gpu_list, ram_list, num_measure_fast_alg, stop_event))
...
try:
    ...
    for i in range(len(one_hot_batch_list)):  
        ...
        for r in range(num_of_repeat_fast_alg):  
            ...
            tf.print(f'Matrix Mult Log Version, Input Length: {L}, Repetition {r + 1} of {num_of_repeat_fast_alg} completed.')
        #tf.print(alpha)
        #tf.print(grads)
...
save_data(save_dir, comp_time, gpu_list, ram_list, 'matrix_mult_log_tf')