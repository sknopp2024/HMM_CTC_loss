...
# Script for running pHMM diagonal vectorised version
...
from _pHMM_diag_vectorised_tf import pHMM_diag_vectorised
from _generate_data_pHMM import generate_data_pHMM

#path in which data should be saved
save_dir = os.path.join(homefolder, "Programs", "results_data_plots",
    "batchsize_8_gradient", "pHMM")
#%%
# Generate data
(A_insert_cols, A_match_cols, A_delete_cols,
 B_list, pi_list,
 input_batch_list,
 one_hot_batch_list) = generate_data_pHMM(sparse_transition_matrix = True)
...
num_of_repeat_slow_alg = 1

num_measure_fast_alg = 1  # Measure GPU and RAM 5 times per second
num_measure_slow_alg = 1  # Measure GPU and RAM once per second

#%%
@tf.function
def compute_loss_and_grads(A_insert, A_match, A_delete, B, pi, u, U):
    '''
    ...
    with tf.GradientTape() as tape:
        ...
        alpha = pHMM_diag_vectorised(A_insert, A_match, A_delete, B, pi, u, U)  
        
        # Calculate Negative Log-Likelihood
        last_log_alpha = alpha[:, :, -1]  # shape: (batchsize, K)
        loss = -tf.reduce_logsumexp(last_log_alpha, axis=1)  # shape: (batchsize,)
    
    grads = tape.gradient(loss, [A_insert, A_match, A_delete, B, pi])  # Gradient
...
comp_time = np.zeros((len(one_hot_batch_list), num_of_repeat_fast_alg), dtype = float)  
gpu_list = []
ram_list = [] 
...
try:
    ...
    for i in range(len(one_hot_batch_list)):  
        ...
        for r in range(num_of_repeat_fast_alg):  
            ...
            alpha, loss, grads = compute_loss_and_grads(A_insert_cols[i], 
                                                        A_match_cols[i], 
                                                        A_delete_cols[i], 
                                                        B_list[i], pi_list[i], 
                                                        input_batch_list[i], 
            ...
            comp_time[i, r] = end - start
            
            # Print progress message
            tf.print(f'pHMM diagonal vectorised Version, Input Length: {L}, Repetition {r + 1} of {num_of_repeat_fast_alg} completed.')
        #tf.print(alpha)
        #tf.print(grads)
...
#save data
save_data(save_dir, comp_time, gpu_list, ram_list, 'diag_vec_tf')