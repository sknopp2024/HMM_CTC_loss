...
# Forward Alg with nested for-loops, not in log-space, underflow

import tensorflow as tf

@tf.function(reduce_retracing=True) 
def nested_for_loops_unstable(A, B, pi, u, U):
    """
    Sequential forward algorithm with for-loops to compute the forward probabilities (alpha).
    ...
    alpha_final = tf.transpose(alpha.stack(), perm=[1, 2, 0])  # (batch x K x L)
...
F_alg3 = nested_for_loops_unstable(A, B, pi_batch, u, U)
with np.printoptions(precision = 5, suppress = True, linewidth = 100):
    print (F_alg3)
print(tf.argmax(F_alg3, axis=1))
print(tf.math.log(tf.reduce_sum(F_alg3[:, :, -1], axis = 1)))      
...
with tf.GradientTape() as tape:
    ...
    alpha = nested_for_loops_unstable(A, B, pi_batch, u, U)  # (batch x K x L)
    final_prob = tf.math.log(tf.reduce_sum(alpha[:, :, -1], axis = 1))
    loss = -tf.reduce_mean(final_prob)  # Negative Log-Likelihood
