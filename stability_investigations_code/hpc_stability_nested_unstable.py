...
os.chdir(sub_folder)
from _nested_for_loops_unstable import nested_for_loops_unstable
...
def compute_loss_and_grads(A, B, pi, u, U):
    ...
    with tf.GradientTape() as tape:
        ...
        alpha = nested_for_loops_unstable(A, B, pi, u, U)  # (batch x K x L)
...
gpu_monitor_thread = threading.Thread(target = gpu_ram_measure, 
                                      args = (gpu_list_0, ram_list_0, num_measure_slow_alg, stop_event))
...
try:
    ...
    for i in range(len(one_hot_batch_list)):  
        ...
        for r in range(num_of_repeat_slow_alg):  
            ...
            tf.print(f'Nested unstable, Input Length: {L}, Repetition {r + 1} of {num_of_repeat_slow_alg} completed.')
        #tf.print(alpha)
        #tf.print(grads)
...
save_data(save_dir, comp_time_0, gpu_list_0, ram_list_0, 'nested_unstable')
