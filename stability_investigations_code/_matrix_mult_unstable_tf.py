...
# Forward Alg matrix multiplication, not in log-space, underflow

import tensorflow as tf

@tf.function(reduce_retracing=True) 
def matrix_mult_unstable(A, B, pi, U):
    """
    ...
    alpha = alpha.write(0, alpha_0)  # (L x batchsize x K)
    
    # Perform the forward computation for each time step (t = 1 to L-1)
    alpha_t = alpha_0
    for t in tf.range(1, L):
        alpha_t = tf.matmul(alpha_t, A) * UB[:, t, :]  # Compute forward prb
        alpha = alpha.write(t, alpha_t)
    
    # Stack and transpose: (L x batchsize x K) -> (batchsize x K x L)
    alpha_stacked = alpha.stack()
    alpha_final = tf.transpose(alpha_stacked, perm=[1, 2, 0])
...
F_alg1 = matrix_mult_unstable(A, B, pi_batch, U)
with np.printoptions(precision = 5, suppress = True, linewidth = 100):
    print(F_alg1)
print(tf.argmax(F_alg1, axis=1))
print(tf.math.log(tf.reduce_sum(F_alg1[:, :, -1], axis = 1)))  # log likelihood
...
with tf.GradientTape() as tape:
    ...
    alpha = matrix_mult_unstable(A, B, pi_batch, U)  # (batch x K x L)
