#Sara Knopp, last modified on 07.07.2025

# CTC HMM, vectorised nested
...
from _CTC_HMM_vectorised_nested_tf import CTC_HMM_vectorised_nested
...
def compute_loss_and_grads(labels, logits, num_frames, blank):
    ...
    with tf.GradientTape() as tape:
        ...
        # Apply the algorithm
        CTC_loss = CTC_HMM_vectorised_nested(labels, logits, num_frames, blank)
...
comp_time_for = np.zeros((len(labels_list), num_of_repeat_fast_alg), dtype=float)  
gpu_list_for = []
ram_list_for = [] 
results_for = []
...
gpu_monitor_thread = threading.Thread(target=gpu_ram_measure, 
                                      args=(gpu_list_for, ram_list_for, 
...
try:
    ...
    for i in range(len(labels_list)):  
        ...
        for r in range(num_of_repeat_fast_alg):  
            ...
            comp_time_for[i, r] = end - start
            
            # Print progress message
            tf.print(f'Improved Subdiagonal For Loop CTC HMM, Input Length: {L}, Repetition {r + 1} of {num_of_repeat_fast_alg} completed.')
        #tf.print(grads)
        tf.print('CTC loss:', CTC_loss)
        results_for.append(CTC_loss)
finally:
...
# Save data    
save_data(save_dir, comp_time_for, gpu_list_for, ram_list_for, 'subdiag_for_improve_tf') 

# Save results
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

algorithm_index = 'subdiag_for_improve'
