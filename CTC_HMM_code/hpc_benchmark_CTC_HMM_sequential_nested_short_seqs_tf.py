...
from _CTC_HMM_sequential_nested_tf import CTC_HMM_nested_sequential 
...
def compute_loss_and_grads(labels, logits, num_frames, blank):
    ...
    # Apply the algorithm
    CTC_loss = CTC_HMM_nested_sequential(labels, logits, num_frames, blank)
...
comp_time_nest = np.zeros((len(labels_list), num_of_repeat_slow_alg), dtype=float)  
gpu_list_nest = []
ram_list_nest = [] 
results_nest = []
...
gpu_monitor_thread = threading.Thread(target=gpu_ram_measure, 
                                      args=(gpu_list_nest, ram_list_nest, 
    ...
    for i in range(len(labels_list)-1): 
        ...
        for r in range(num_of_repeat_slow_alg):  
            ...
            CTC_loss, grads = compute_loss_and_grads(labels_list[i], logits_list[i], 
                                      num_frames[i], blank)
            # Apply the algorithm
            # CTC_loss = compute_loss_and_grads(labels_list[i], logits_list[i], 
            #                           num_frames[i], blank)
            ...
            comp_time_nest[i, r] = end - start
            
            # Print progress message
            tf.print(f'Nested CTC_HMM, Input Length: {L}, Repetition {r + 1} of {num_of_repeat_slow_alg} completed.')
        #tf.print(grads)
        tf.print('CTC loss:', CTC_loss)
        results_nest.append(CTC_loss)
finally:
...
# Save data
save_data(save_dir, comp_time_nest, gpu_list_nest, ram_list_nest, 'nest_short_seqs_tf')

# Save results
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

algorithm_index = 'nest_short_seqs'
...
try:
    with open(results_file, 'w') as comp_file:
        comp_file.write(f"{results_nest}")
