...
# Improve Subdiagonal Version of CTC_HMM by writing a function for calculating
# UB stepwise to save computational space
...
from _CTC_HMM_vectorised_scan_tf import CTC_HMM_vectorised_scan
...
def compute_loss_and_grads(labels, logits, num_frames, blank):
    ...
    with tf.GradientTape() as tape:
        ...
        # Apply the algorithm
        CTC_loss = CTC_HMM_vectorised_scan(labels, logits, num_frames, blank)
...
comp_time_scan = np.zeros((len(labels_list), num_of_repeat_fast_alg), dtype=float)  
gpu_list_scan = []
ram_list_scan = [] 
results_scan = []
...
try:
    ...
    for i in range(len(labels_list)):  
        ...
        for r in range(num_of_repeat_fast_alg):  
            ...
            tf.print(f'Improved Subdiagonal Scan CTC_HMM, Input Length: {L}, Repetition {r + 1} of {num_of_repeat_fast_alg} completed.')
        #tf.print(grads)
...
# Save data
save_data(save_dir, comp_time_scan, gpu_list_scan, ram_list_scan, 'subdiag_scan_improve_tf')   

# Save results
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

algorithm_index = 'subdiag_scan_improve'
