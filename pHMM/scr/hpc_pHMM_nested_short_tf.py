...
# Script for running pHMM not diagonal nested For-Loops version
...
from _pHMM_nested_tf import pHMM_nested_for
...
#%%
# Generate data
(A_list, B_list, pi_list, input_batch_list, one_hot_batch_list) = generate_data_pHMM()
...
num_of_repeat_slow_alg = 11

num_measure_fast_alg = 1  # Measure GPU and RAM 5 times per second
num_measure_slow_alg = 1  # Measure GPU and RAM 4 times per second

#%%
@tf.function
def compute_loss_and_grads(A, B, pi, u, U):
    '''
    ...
    with tf.GradientTape() as tape:
        ...
        alpha = pHMM_nested_for(A, B, pi, u, U)  # (batch x K x L)
...
comp_time = np.zeros((len(one_hot_batch_list), num_of_repeat_slow_alg), dtype = float)  
gpu_list = []
ram_list = [] 
...
try:
    ...
    for i in range(len(one_hot_batch_list)-1):  
        ...
        for r in range(num_of_repeat_slow_alg):  
            ...
            alpha, loss, grads = compute_loss_and_grads(A_list[i], B_list[i], 
                                                        pi_list[i], input_batch_list[i], 
            ...
            tf.print(f'pHMM not diagonal nested for loop Version, Input Length: {L}, Repetition {r + 1} of {num_of_repeat_slow_alg} completed.')
        #tf.print(alpha)
        #tf.print(grads)
...
#save data
save_data(save_dir, comp_time, gpu_list, ram_list, 'not_diag_nested_short_tf')