#Sara Knopp, last modified on 22.07.2025

# Forward Alg matrix multiplication, underflow save version

import tensorflow as tf

@tf.function(reduce_retracing=True) 
def matrix_mult_scaled(A, B, pi, U):
    """
    ...
    Returns:
    alpha, tf.Tensor: Log-scaled forward probabilities `alpha` (batch_size, K, L). 
               dtype=tf.float32.
    scaling, tf.Tensor: Scaling factors used at each time step, (batch_size, L). 
               dtype=tf.float32.
    Notes:
        - K: Number of states
    ...
    Z_0 = tf.reduce_sum(alpha_0, axis = 1)  # Sum of entries in first column per batch
    scaling_factors = scaling_factors.write(0, Z_0) # Update scaling_factors
    
    # Make calculations underflow save
    alpha_0 = alpha_0 / tf.expand_dims(Z_0, axis=1)  # Scale first col of alpha
    ...
    for t in tf.range(1, L):
        alpha_t = tf.matmul(alpha_t, A) * UB[:, t, :]  # Calculation forward alg
    
        Z_t = tf.reduce_sum(alpha_t, axis = 1)
        scaling_factors = scaling_factors.write(t, Z_t) # Update scaling_factors
        alpha_t = alpha_t / tf.expand_dims(Z_t, axis=1)  # Scale alpha_t
        alpha = alpha.write(t, alpha_t)  # Update alpha
    
    # Stack and transpose: (L x batchsize x K) -> (batchsize x K x L)
    alpha_final = tf.transpose(alpha.stack(), perm=[1, 2, 0])
    
    scaling = scaling_factors.stack()  # (L x batchsize) for calc of loglikelihood
...
F_alg1, scaling_matrix_mult = matrix_mult_scaled(A, B, pi_batch, U)
with np.printoptions(precision = 5, suppress = True, linewidth = 100):
    print (F_alg1)
print(tf.argmax(F_alg1, axis=1))
print('Log Likelihood Matrix Mult:\n', tf.reduce_sum(tf.math.log(scaling_matrix_mult), axis=0))  # loglikelihood
...
with tf.GradientTape() as tape:
    ...
    #alpha, scaling = matrix_mult_scaled(A, B, pi_batch, U)  # (batch x K x L)
    #final_log_prob = tf.reduce_sum(tf.math.log(scaling), axis=0)
    
    alpha = matrix_mult_scaled(A, B, pi_batch, U, rescaling=True)  # (batch x K x L)
