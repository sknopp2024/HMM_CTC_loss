#Sara Knopp, last modified on 04.12.2024
...
# algorithms always for the same input size, for example [10, 100, 1000], 
...
#maximum used RAM: up to sample size 10 000 all algs need below 200MB RAM. 
# For alg 0 and alg 1 it remains at this 200 MB. Alg 2, 3.1 and 3.2 need
# around 1200 MB RAM for sample size 100 000 and nearly 7000 MB RAM for 
# sample size 500 000
...
#now tensorflow can just take as much RAM as it needs
...
#os.getcwd()
os.chdir('/home/jovyan/Thesis/')
#os.chdir("C:/Users/Lenovo/Documents/Sara/Uni/Master/5. Semester/Masterarbeit/Programme/")
...
def get_gpu_and_ram_often(gpu_list, ram_list):
    while not stop_event.is_set():
        ...
        #measure RAM and GPU-utilization 1000 times each second 
        time.sleep(0.0001)
    return gpu_list, ram_list
...
def _prep_ram_and_gpu_data(gpu_list, ram_list):
    ...
    for input_size, measurement in ram_list:   #go through measured GPU values
        if input_size not in dict_ram_data: #check, if this size already exists in dict dict_ram_data 
            dict_ram_data[input_size] = [] #if not create empty list for this input size
        
        dict_ram_data[input_size].append(measurement) #add measured RAM
    ...
    for input_size, measurements in dict_ram_data.items():
        max_ram.append(np.max(measurements))
...
#if measurement of GPU and RAM of alg 1 was not possible
#pad variables so that plotting is possible
#mean_gpu_1 = np.pad(mean_gpu_1, (3, 0), 'constant', constant_values = np.nan)
#max_ram_1 = np.pad(max_ram_1, (3, 0), 'constant', constant_values = np.nan)
...
plt.axhline(y = 8192, color = 'gray', linestyle = '--', label = 'Max RAM (8192 MiB)')
plt.grid(True)
plt.legend()
plt.savefig('compare_ram_all_alg_log_log.png') #save plot
plt.show()
