#Sara Knopp, last modified on 18.11.2024
...
#GPU-utilization: all algs have a peak at input size 100, instead of alg 1.
#  Its peak can be observed at 100 000. Alg 1, 2 and 3.1 have the highest 
#  peak with 25%, Alg 3.1's peak is around 8% and alg 0 around 4%. For all
#  other sample sizes the mean of the GPU-utilization is around 1% for all
#  algs
#maximum used RAM: the maximum used RAM of all algs is constant for all input
#  sizes and is between 6900 and 7000 MiB

#comment: GPU and RAM are measured each second. For the really fast algs (1 and 2)
#  we could measure them again more often. Maybe as well for the other algs 
#  for small sample size, because all algs were fast for small input sizes 
...
#%%
#determine computational time, maximal used RAM and GPU-utilization for each algorithm
#using the generated input sequences

num_of_repeat = 3   #repeat each algorithm several times   


#function for measuring RAM and GPU-utilization every second
...
def get_gpu_and_ram(gpu_list, ram_list):
    while not stop_event.is_set():
        ...
        #print('current GPU utilization ', gpu.load * 100)
    
        #measure RAM and GPU-utilization each second 
        time.sleep(1)
...
with open('comp_time_0.txt', 'w') as comp_file, \
    ...
    for i in range(len(one_hot_list)):  #apply algorithm 0 on each input
        #length of current input sequence    
        L = len(one_hot_list[i])
        
        for r in range(num_of_repeat):  #repeat the alg several times for each sequence size      
            #start timer
            start = time.time()
            ...
            print('Algorithm 0, Iteration', i, 'Wiederholung', r, 'beendet')

        #print(gpu_list_0, ram_list_0, comp_time_0)
        #save computational time
        comp_file.write(f"Inputgröße {i}:\n")
        for r in range(num_of_repeat):
            comp_file.write(f"{comp_time_0[i, r]}\n")
        
        #save GPU-utilization
        gpu_file.write(f"Inputgröße {i}:\n")
        for gpu_usage in gpu_list_0:
            gpu_file.write(f"{gpu_usage}\n")
        
        #save used RAM
        ram_file.write(f"Inputgröße {i}:\n")
...
with open('comp_time_1.txt', 'w') as comp_file, \
    ...
    for i in range(len(one_hot_list)):  #apply algorithm 1 on each input
        #length of current input sequence    
        L = len(one_hot_list[i])
        ...
        for r in range(num_of_repeat):  #repeat the alg several times for each sequence size      
            ...
            print('Algorithm 1, Iteration', i, 'Wiederholung', r, 'beendet')

        #print(gpu_list_1, ram_list_1, comp_time_1)
        #save computational time
        comp_file.write(f"Inputgröße {i}:\n")
        for r in range(num_of_repeat):
            comp_file.write(f"{comp_time_1[i, r]}\n")
...
with open('comp_time_2.txt', 'w') as comp_file, \
    ...
    for i in range(len(one_hot_list)):  #apply algorithm 2 on each input
        #length of current input sequence    
        L = len(one_hot_list[i])
        ...
        for r in range(num_of_repeat):  #repeat the alg several times for each sequence size      
            ...
            print('Algorithm 2, Iteration', i, 'Wiederholung', r, 'beendet')

        #print(gpu_list_2, ram_list_2, comp_time_2)
        #save computational time
        comp_file.write(f"Inputgröße {i}:\n")
        for r in range(num_of_repeat):
            comp_file.write(f"{comp_time_2[i, r]}\n")
...
with open('comp_time_3_1.txt', 'w') as comp_file, \
    ...
    for i in range(len(one_hot_list)):  #apply algorithm 3_1 on each input
        #length of current input sequence    
        L = len(one_hot_list[i])
        ...
        for r in range(num_of_repeat):  #repeat the alg several times for each sequence size      
            ...
            print('Algorithm 3.1, Iteration', i, 'Wiederholung', r, 'beendet')

        #print(gpu_list_3_1, ram_list_3_1, comp_time_3_1)
        #save computational time
        comp_file.write(f"Inputgröße {i}:\n")
        for r in range(num_of_repeat):
            comp_file.write(f"{comp_time_3_1[i, r]}\n")
...
with open('comp_time_3_2.txt', 'w') as comp_file, \
    ...
    for i in range(len(one_hot_list)):  #apply algorithm 3.2 on each input
        #length of current input sequence    
        L = len(one_hot_list[i])
        ...
        for r in range(num_of_repeat):  #repeat the alg several times for each sequence size      
            ...
            F_alg3_2 = forward_parallel_scan_associative_sparse(A, B, pi, one_hot_list[i])
            
            # stop timer 
            end = time.time()
            
            comp_time_3_2[i, r] = end - start
            print('Algorithm 3.2, Iteration', i, 'Wiederholung', r, 'beendet')

        #print(gpu_list_3_2, ram_list_3_2, comp_time_3_2)
        #save computational time
        comp_file.write(f"Inputgröße {i}:\n")
        for r in range(num_of_repeat):
            comp_file.write(f"{comp_time_3_2[i, r]}\n")
...
#prepared data for plotting, mean of GPU-utilization and maximum used RAM
...
#%% 
#visualisation of computational time
