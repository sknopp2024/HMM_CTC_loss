# Sara Knopp, last modified on 30.09.2025

# Benchmark HMMs, scan, but utilizing the sparsity of A 

import tensorflow as tf
from tensorflow_probability.python.math.scan_associative import scan_associative

@tf.function(reduce_retracing=True)
def associative_scan_sparse_scaled(A, B, pi, U):
    ''' 
    Forward algorithm computed in parallel using scan_associative, 
    leveraging the sparsity of the transition matrix A.
    ...
    # Step 1: Precompute emission probabilities
    # U: (batch_size, L, H), B: (K, H) → UB: (batch_size, L, K)
    UB = tf.matmul(U, B, transpose_b=True)
    
    # Step 2: Construct a sparse representation of A 
    # Masks all non-zero transitions in A to identify allowed predecessor states.
    nonzero_mask = tf.not_equal(A, 0.0)
    indices = tf.where(nonzero_mask)  # Shape: [num_nonzero, 2]
    ...
    A_sparse = tf.sparse.SparseTensor(indices=indices, values=values, 
                                      dense_shape=tf.shape(A, out_type=tf.int64))

    #function that replaces tf.einsum() 
    def compute_M(b):
        
        # TensorArray to collect matrices
        matrix_array = tf.TensorArray(dtype=tf.float32, size=L-1)
        
        def body(t, M_t_prev_list):
            M_t = _compute_M_t(A_sparse, UB[b, t + 1])  # scale column wise
            M_t_dense = tf.sparse.to_dense(M_t)

            M_t_prev_list = M_t_prev_list.write(t, M_t_dense)
            return t + 1, M_t_prev_list

        # tf.while_loop for t in range(L-1)
        _, M_t = tf.while_loop(
            lambda t, _: t < L - 1,
            body,
            loop_vars=[0, matrix_array]
        )
        
        return M_t.stack()  # Shape: (L-1, K, K)

    # Step 3: Compute the inputs for the associative scan (M) (batchsize, L-1, K, K)
    M = tf.map_fn(fn = compute_M, elems = tf.range(batchsize),
        fn_output_signature = tf.TensorSpec(shape = (None, None, None), dtype=tf.float32)
    ) 
    
    # Step 4: Calculate initial state M_0
    pi_ = tf.expand_dims(pi, 1)  # Ensure type compatibility
    UB_ = tf.expand_dims(UB[:, 0, :], 1)  # Extract the first time step of UB
    M_0 = tf.expand_dims(tf.tile(pi_ * UB_, multiples = [1, 15, 1]), 1)  
    ...
    # Step 5: Prepend the initial state to M
    M_complete = tf.concat([M_0_scaled, M], axis = 1) # (b x L x K x K)
    ...
    # Step 6: Compute forward probabilities in parallel using scan_associative
    F_all, scalings = scan_associative(func, M_scaling, axis = 1)  # (batch_size x L x K x K)
    
    # Step 7: Extract the forward probabilities for each time step
    F = F_all[:, :, 0, :]  
...
def _compute_M_t(A_sparse, scale_vector):
    """
    Scales each column of a sparse matrix by the corresponding element in a 
    scale vector.
    
    Args:
        A_sparse (tf.sparse.SparseTensor): Sparse matrix of shape (K, K).
        scale_vector (tf.Tensor): 1D tensor of length K containing the scaling 
                                  factors for each column.
    
    Returns:
        tf.sparse.SparseTensor: A new sparse matrix where each column of A_sparse 
                               is scaled by the corresponding element in scale_vector.
    """
    # A_sparse.indices[:, 1] are the column indices
    M_t = A_sparse.values * tf.gather(scale_vector, A_sparse.indices[:, 1])
...
with tf.GradientTape() as tape:
    ...
    alpha, scalings = associative_scan_sparse_scaled(A, B, pi_batch, U)  # (batch x K x L)
