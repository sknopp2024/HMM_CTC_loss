...
from _scan_sparse_tf import associative_scan_sparse_scaled 
...
def compute_loss_and_grads(A, B, pi, U):
    ...
    with tf.GradientTape() as tape:
        ...
        alpha, scalings = associative_scan_sparse_scaled(A, B, pi, U)  # (batch x K x L)
        
        # Calculate Negative Log-Likelihood
        loss = tf.squeeze(scalings[:, -1], axis = 2)
...
try:
    ...
    for i in range(len(one_hot_batch_list)):  
        ...
        for r in range(num_of_repeat_fast_alg):  
            ...
            tf.print(f'Algorithm Associative Scan Sparse, Input Length: {L}, Repetition {r + 1} of {num_of_repeat_fast_alg} completed.')
        #tf.print(alpha)
        #tf.print(grads)
...
#save data
save_data(save_dir, comp_time, gpu_list, ram_list, 'asso_scan_sparse_tf')