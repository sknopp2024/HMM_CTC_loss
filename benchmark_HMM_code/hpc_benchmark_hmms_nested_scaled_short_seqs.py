...
subfolder = "benchmark_HMM_code"
os.chdir(subfolder)  

from _nested_for_loops_scaled import nested_for_loops_scaled
...
save_dir = os.path.join(homefolder, "Programs", "results_data_plots",
    "batchsize_8_gradient", "benchmark_HMMs")
#%%
# Generate data
...
def compute_loss_and_grads(A, B, pi, u, U):
    ...
    with tf.GradientTape() as tape:
        ...
        alpha, _ = nested_for_loops_scaled(A, B, pi, u, U)  # (batch x K x L)
...
try:
    ...
    for i in range(len(one_hot_batch_list)-1):  
        ...
        for r in range(num_of_repeat_fast_alg):  
            ...
            alpha, loss, grads = compute_loss_and_grads(A, B, pi, input_batch_list[i],
            ...
            tf.print(f'Algorithm Nested For-Loops, Input Length: {L}, Repetition {r + 1} of {num_of_repeat_slow_alg} completed.')
        #tf.print(alpha)
        #tf.print(grads)
...
#save data
save_data(save_dir, comp_time, gpu_list, ram_list, 'nested_scaled_short')